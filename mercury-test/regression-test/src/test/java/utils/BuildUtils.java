...
public class BuildUtils {
  public static void transferFreeCkb(String fromAddress, String toAddress, BigInteger amount)
      throws IOException {
    System.out.println(
        "from: " + fromAddress + ", to: " + toAddress + ", amount: " + amount + " free CKB");
    TransferPayloadBuilder builder = new TransferPayloadBuilder();
    builder.assetInfo(AssetInfo.newCkbAsset());
    builder.from(
        From.newFrom(
            Arrays.asList(ItemFactory.newIdentityItemByAddress(fromAddress)), Source.Free));
  ...
  public static void transferFreeUdt(String fromAddress, String toAddress, BigInteger amount)
      throws IOException {
    System.out.println(
        "from: " + fromAddress + ", to: " + toAddress + ", amount: " + amount + " free UDT");
  ...
  public static void generateClaimableUdt(String fromAddress, String toAddress, BigInteger amount)
      throws IOException {
    System.out.println(
        "from: "
            + fromAddress
  ...
  public static void consumeClaimableUdt(String fromAddress, String toAddress, BigInteger amount)
      throws IOException {
    BuildUtils.ensureOneAcp(toAddress);
    System.out.println(
        "from: "
            ...
            + ", amount: "
            + amount
            + " consumed claimable UDT");
    TransferPayloadBuilder builder = new TransferPayloadBuilder();
    builder.assetInfo(AssetInfo.newUdtAsset(UdtHolder.UDT_HASH));
    builder.from(
        From.newFrom(
            Arrays.asList(ItemFactory.newIdentityItemByAddress(fromAddress)), Source.Claimable));
    builder.to(To.newTo(Arrays.asList(new ToInfo(toAddress, amount)), Mode.HoldByTo));
    // testAddress1 is used to pay fee if necessary.
    builder.payFee(AddressWithKeyHolder.testAddress1());
    TransactionCompletionResponse s = ApiFactory.getApi().buildTransferTransaction(builder.build());
  ...
  public static void adjustAcpNumber(String fromAddress, String target_address, int amount)
      throws IOException {
    System.out.println(
        "address "
            + target_address
            + " create "
            + amount
            + " acp cell, acp address: "
            + AddressTools.generateAcpAddress(target_address));
      ...
      try {
        ...
        System.out.println(
            txHash
                + " rpc status: "
                + status
                + ", tip block number: rpc-"
                + rpcTipBlockNumber
                + ", mercury-"
                + mercuryTipBlockNumber);
  ...
  public static void generateTestAddresses(String testAddressesPath, int number) throws Exception {
    ...
    for (int i = 0; i < number; i++) {
      AddressTools.AddressGenerateResult testAddress =
  ...
  public static ArrayList<AddressTools.AddressGenerateResult> readTestAddresses(
      String testAddressesPath) {
    Path path = Paths.get(testAddressesPath);
    ...
    try {
      ...
      for (int i = 0; i < array.size(); i++) {
        ...
        testAddress.address =
            array.get(i).getAsJsonObject().get("address").toString().replaceAll("\"", "");
        testAddress.lockArgs =
            array.get(i).getAsJsonObject().get("lockArgs").toString().replaceAll("\"", "");
        testAddress.privateKey =
            array.get(i).getAsJsonObject().get("privateKey").toString().replaceAll("\"", "");
        // Because mercury return short_format address in SignatureAction, so here we must use short
        // format.
        ECKeyPair ecKeyPair =
            ECKeyPair.createWithPrivateKey(Numeric.toBigInt(testAddress.privateKey), true);
        String publicKey = ecKeyPair.getPublicKey().toString(16);
        String shortAddress =
            AddressTools.convertPublicKeyToShortAddress(Network.TESTNET, publicKey);
  ...
  public static void ensureCkbFreeBalance(String address, BigInteger freeAmount)
      throws IOException {
    ...
    if (transferAmount.compareTo(BigInteger.ZERO) != 0
        && transferAmount.compareTo(AmountUtils.ckbToShannon(61)) == -1) {
      System.out.println(
          "Address "
              + address
              + " clear "
              + freeBalance
              + " CKB, because transfer amount "
              + transferAmount
              + " less than minimum transferable CKB");
  ...
  void testEnsureCkbFreeBalance() {
    try {
      ArrayList<AddressTools.AddressGenerateResult> as =
          BuildUtils.readTestAddresses(PrepareTransfer.testAddressesPath);
  ...
  public static void ensureUdtFreeBalance(String address, BigInteger freeAmount)
      throws IOException {
    List<BalanceResponse> balances = BuildUtils.getBalance(address);
    BigInteger freeBalance = BigInteger.ZERO;
    for (BalanceResponse balance : balances) {
      if (balance.assetInfo.assetType.equals(AssetType.UDT)
          && balance.assetInfo.udtHash.equals(UdtHolder.UDT_HASH)) {
        freeBalance = freeBalance.add(balance.free);
      }
    }
  ...
  public static void ensureUdtClaimableBalance(String address, BigInteger claimableAmount)
      throws IOException {
    List<BalanceResponse> balances = BuildUtils.getBalance(address);
  ...
  public static boolean ensureDaoDeposit(String address, BigInteger minDepositAmount)
      throws IOException {
  ...
  public static boolean ensureDaoWithdraw(String address, BigInteger depositAmount)
      throws IOException {
    String lockArgs = AddressWithKeyHolder.getPubKeyByAddress(address);
    List<CellResponse> cells = BuildUtils.getDaoCells(lockArgs);
    ...
    if (!hasMaturityDaoWithdraw) {
      for (CellResponse cell : cells) {
        ...
        if (cell.outputData.equals("0x0000000000000000")) {
          ...
          if (tipEpochNumber.subtract(daoDepositEpochNumber).compareTo(BigInteger.valueOf(5))
              == 1) {
  ...
  void testEnsureDaoWithdraw() {
    try {
      System.out.println(
          BuildUtils.ensureDaoWithdraw(
              AddressWithKeyHolder.testAddress2(), AmountUtils.ckbToShannon(200)));
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  void supplyCkb() throws IOException {
    BuildUtils.transferFreeCkb(
        AddressWithKeyHolder.testAddress2(),
        AddressWithKeyHolder.testAddress0(),
        AmountUtils.ckbToShannon(1000));
  }
}
